    ...
    do {
        ...
        if (escolha == 1) {
            ...
            for (int i = 0; i < numero_de_faixas; i++) {
                ...
                else if (strcmp(cor, "ausente") == 0) resistor.cores[i] = ausente; // Nova cor representando 20% de tolerância
                else {
                    printf("Cor inválida!\n");
                    deu_certo = system("clear");
                    i--; 
            ...
            if (numero_de_faixas == 4) {
                ...
                tolerancia = (resistor.cores[3] == dourado) ? 5 : (resistor.cores[3] == prata) ? 10 : 20; // Alterando para 20% de tolerância se a cor for "ausente"
            } else {
                valor_base = resistor.cores[0] * 100 + resistor.cores[1] * 10 + resistor.cores[2];
                multiplicador = pow(10, resistor.cores[3]);
                tolerancia = (resistor.cores[4] == dourado) ? 5 : (resistor.cores[4] == prata) ? 10 : 20; // Alterando para 20% de tolerância se a cor for "ausente"
            }
